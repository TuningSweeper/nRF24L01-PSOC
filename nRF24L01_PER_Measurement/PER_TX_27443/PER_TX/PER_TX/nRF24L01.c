/* Routines for handling nRF24L01 RF Transceiver for PSoC 
 *
 * (C) Juho Vesanen, Released under MIT license.
 */
...
void nRF24L01_commonConfiguration(void){
	...
	nRF24L01_sendInstruction( W_REGISTER | RF_CH, 0x25 );

	// dynamic payload
	nRF24L01_sendInstruction( W_REGISTER | DYNPD, 0x0f);
	nRF24L01_sendInstruction( W_REGISTER | FEATURE, 0x04);

	// set data rate to 1Mbps
	nRF24L01_sendInstruction( W_REGISTER | RF_SETUP, 0x06 );

	// set RX_PW_P0 to 2 bytes 
	nRF24L01_sendInstruction( W_REGISTER | RX_PW_P0, 10 );

	// Three byte addresses
	nRF24L01_sendInstruction( W_REGISTER | SETUP_AW, 0x01);

	// set CRC to 2 bytes in the CONFIG and disable interrupts on IRQ
	nRF24L01_sendInstruction( W_REGISTER | CONFIG, 0b01111110 );
...
void nRF24L01_setRxTxAddr(void){
	...
	BYTE RxAddrP0[3] = { 0x12, 0x34, 0x56};
	BYTE TxAddr[3] = { 0x12, 0x34, 0x56};
	
	// set the RX_ADDR_P0, 3 bytes
	nRF24L01_sendPayload( W_REGISTER | RX_ADDR_P0, RxAddrP0, 3);
	
	// set the TX_ADDR, 3 bytes
	nRF24L01_sendPayload( W_REGISTER | TX_ADDR, TxAddr, 3);
}


// set the operating mode of the transceiver
void nRF24L01_setMode(char mode) {
	BYTE presentConfig;
